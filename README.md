...
|---|------|------|
| 1 | RSI(7) < 35 | 当前处于超卖区间 |
| 2 | EMA(99) 斜率 > 0 | 要求 EMA99 处于上行趋势才买入（防接飞刀，10根×1分钟=10分钟窗口） |
...
|--------|------|------|----------|
| 1 | RSI(7) > 80 | 恐慌卖出 | ✅ 启用 |
| 2 | EMA(99) 斜率 < -1% | 趋势退出 (V5-40) | ✅ 启用 |
| 3 | RSI(7) 下穿 70 | 全仓卖出 | ✅ 启用 |
| 4 | 涨幅 ≥ +100% | 固定止盈 | ✅ 启用 |
| 5 | 上涨 ≥ +30% 后回撤 -20% | 移动止损 | ✅ 启用 |
| 6 | 跌幅 ≤ -50% | 固定止损 | ✅ V5-26 启用 |
| 7 | 量能萎缩(最近3根均量 < 前4根均量×0.3) | 量能出场 | ✅ V5-26 启用 |
| 8 | 持仓 ≥ 6 小时 | 超时卖出 (TIMEOUT_EXIT) | ✅ 启用 |

> **当根 K 线买入保护**（V5-41）：买入后这根 K 线收盘前不触发 RSI 类卖出（防同根 K 线 V 反洗出）；下根 K 线开始正常监控。
...
| `EMA_SLOPE_MIN_PCT` | `0` | 斜率下限（%），≥ 0 才允许买入（要求上行） |
| `EMA_SLOPE_EXIT_ENABLED` | `true` | V5-40: 持仓中斜率跌破阈值立即卖出 |
| `EMA_SLOPE_EXIT_PCT` | `-1` | V5-40 退出阈值（%），slope < -1% 立即卖出 |
...
- **V5-39.1**: 修日志格式 bug — V5-39 用了 `%+.2f` 和 `%g` 这种 C printf 格式, winston 的 splat 不支持, 日志显示成字面字符串而非数值. 改成模板字符串 `${...}` 风格, 不影响判断逻辑（判断走 JS 数值比较），仅影响日志可读性
- **V5-40**: ★ EMA99 斜率退出 — 持仓中 EMA99 斜率跌破 EMA_SLOPE_EXIT_PCT (默认 -2%) 立即卖出. 趋势加速下行时抢在硬止损 -50% 之前出场. LOOKBACK 复用买入侧的 EMA_SLOPE_LOOKBACK (10 根×1min=10min 窗口). 阈值比买入侧 -1% 更宽, 给持仓些容忍空间避免震荡频繁出场. ⚠️ 这是策略变更, 未做回测验证, 可能导致震荡假信号被洗出. 不合适随时 .env 改 EMA_SLOPE_EXIT_ENABLED=false 关闭
- **V5-41**: ★ 修同根 K 线买入又卖出 bug (M32 案例) — 08:42 大阴线进行中 BUY (RSI=23.6), 同根 K 线收盘后 closed RSI 89.7→23.0 触发 RSI_CROSS_DOWN_70 同根 K 线卖出. 根因: monitor.js 三条独立的 RSI 卖出路径 (主轮询 closed K 线下穿 / 主轮询 RSI_PANIC / WS 实时下穿) + rsi.js evaluateSignal 的卖出条件 都没检查 lastBuyCandle. 修法: 全部加上 `lastCandleTs !== lastBuyCandle` (主轮询路径) 或 buyTime 时间戳判断 (WS 路径) 的"同根K线买入保护", 给V反一根K线的容忍, 下根 K 线开始 RSI 类卖出恢复正常
- **V5-42**: 斜率参数对称化 — EMA_SLOPE_MIN_PCT -1 → 0 (买入侧严格要求 EMA99 上行才进场); EMA_SLOPE_EXIT_PCT -2 → -1 (持仓侧轻微下行就退出). 配合形成"上行才买、下行就跑"的对称趋势过滤. ⚠️ 比之前更严格, 出单率会显著降低, 但每单胜率应该改善. 观察一两天判断收益变化
